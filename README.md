...
- [ ] 读完 [Anthropic: Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)。  
       ...
       何时使用：原则是尽可能的使用简单的方案，对于明确的任务，工作流可可预测，结果一致性。通常任务用RAG+LLM就足够  
       关于框架：框架好用但是封装太多，建议直接调用LLM API
       增强型LLM：智能体的基本块，模式就是LLM + RAG + tool + memory ，我理解为就是核心就是这个搭子，LLM外挂定制的数据库，完成特定任务的决策推理,这个就是工作流下的LLM的基本单元
      <img width="415" height="200" alt="image" src="https://github.com/user-attachments/assets/22b03303-acfc-4ecc-805d-6b61fbb4448e" />
      常见的工作流模式：
      1.提示链：链式工作流，将任务分解为一系列步骤，在子任务间增加GATE（检查中间步骤是否正常输出）  使用场景：任务可分解为固定子任务，让每个更简单的子任务通过LLM来完成，提升准确性
      2.路由模式：先分类（LLM分类或者传统分类）任务类型，然后走各自的任务   使用场景：存在不同类型的复杂任务，针对单一任务优化，会损害其他的任务输入的效果，例如客服机器人这种，遇到客户问题先分类（一般问题，退款请求，技术支持等）
      3.并行模式：两种方式：分段-任务划分为多个独立子任务，多个LLM同时处理。 投票- 一个任务跑多次（不同模型或者不同提示词）获得不同的输出  使用场景：划分平行子任务提高速度或者多角度提升结果置信度
      4.编排器-工作者：中央LLM（编排器）动态分解任务，将子任务委托给工作者llm,然后总和结果，与并行的区别在于并行是预先定义的子任务，但是编排这是动态出来的，根据输入来进行分解的。 使用场景：无法预测子任务复杂程度
      5.评估期-优化器：这个感觉就是REFLECTION模式，一个LLM生成结果，另一个LLM评估结果，给出反馈（拒绝，反馈，或者接受。。。）  使用场景：有明确的评估标准，通过迭代赶紧或者可衡量标准
      智能体模式：
      <img width="495" height="236" alt="image" src="https://github.com/user-attachments/assets/6240ae7e-807c-4049-b020-bab7eb4eb2b9" />  
      狭义的智能体，LLM能力提升，简单模式完成复杂任务，通常只是LLM循环中根据环境反馈来使用工具，在故障点或者检查点获取人类反馈，有停止条件->工具集以及工具的说明文档需要非常清晰和全面 使用场景：开发问题，难以预测需要的步骤数，无固定路径。  
      <img width="657" height="78" alt="image" src="https://github.com/user-attachments/assets/62a9a682-b730-4194-8a11-1d9b9ce91770" />
      仅在证明可以提升结果时再增加复杂度！  
      构造智能体的核心原则：1.保持简单 2.明确显示智能体规划，为了透明度 3.通过工具文档和测试来仔细设计你的智能体-计算机接口
      工具定义和规范应该像你的整体提示一样受到提示工程的重视
...
- [ ] 读完 [OpenAI: A practical guide to building agents](https://openai.com/business/guides-and-resources/a-practical-guide-to-building-ai-agents/)。
      智能体定义：独立完成任务的系统，1.通过LLM来管理工作流的执行，作出决策 2.可访问多种工具，与外部交互，可动态选择工具，在明确的防护边界内运行  
      什么时候使用智能体：涉及复杂决策，规则集难以维护，高度依赖非结构化数据，即传统确定性的基于规则的方法难以胜任的工作流程  
      智能体设计的基础组件：LLM模型，工具，指令（智能体行为指导以及边界）  1.选择LLM模型：先用能力最强模型的建立性能基线，然后替换为较小模型看是否能到达可接受结果  2.工具：主要分三类-获取数据的工具-行动工具-编排工具，工具要做好标准化定义以及详尽的不重复的文档  
      配置指令：prompt，好的指令少歧义，提升智能体决策能力，保证工作流顺畅执行 好的实践：利用现有的操作手册或文档作基础来构建行为指导，将任务分解成更小更清晰的步骤以减少歧义，为agent每一步行动定义清晰的具体的输出或操作，边界处理，预见过程中可能出现的意外或者错误情况给出分支或替代步骤  
      编排模式：有了基础组件之后，如何组织执行工作流  1.单智能体系统：配备必要的工具和指令，需要有run的概念，通常通过循环来执行任务，达到预定条件后退出  
      2.多智能体：合适考虑多智能体？优先最大化单智能体的能力，当单一智能体难以应对复杂逻辑或庞大工作集（数量庞大，通过详细的工具说明也无法提升性能）时，就要用多智能体了。管理者模式：存在中心管理智能体，通过工具调用协调多个专项智能体工作，并整合结果。  去中心化模式：多个智能体对等工作，通过交接函数（调用工具函数），将任务从一个智能体委托给另一个智能体  
      防护措施：构建防护措施+规划人工干预
