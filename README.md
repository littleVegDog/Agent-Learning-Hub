...
- [ ] 读完 [Anthropic: Building effective agents](https://www.anthropic.com/engineering/building-effective-agents)。  
      ...
      4.编排器-工作者：中央LLM（编排器）动态分解任务，将子任务委托给工作者llm,然后总和结果，与并行的区别在于并行是预先定义的子任务，但是编排这是动态出来的，根据输入来进行分解的。
      5.评估期-优化器：这个感觉就是REFLECTION模式，一个LLM生成结果，另一个LLM评估结果，给出反馈（拒绝，反馈，或者接受。。。）
